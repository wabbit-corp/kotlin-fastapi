...
fun <T : Any> client(kClass: KClass<T>, uri: String): T {
    ...
    for (method in kClass.declaredFunctions) {
        ...
        require(method.annotations.count { it is Http.Get || it is Http.Post || it is Http.Put || it is Http.Delete || it is Http.Patch || it is Http.Head || it is Http.Options } == 1) {
            "Method $fullMethodName has more than one annotation"
        }

        val annotation = method.annotations.first { it is Http.Get || it is Http.Post || it is Http.Put || it is Http.Delete || it is Http.Patch || it is Http.Head || it is Http.Options }
        ...
        val streamInParam = vparams.firstOrNull { it != jsonBodyParameter && isStreamingType(it.type) }
        ...
        for ((ord, p) in vparams.withIndex()) {
            ...
            p.annotations.filterIsInstance<Http.Header>().firstOrNull()?.let { headerParams[ord] = it.name }
        ...
        val argIndex: Map<String, Int> = vparams
            .withIndex()
            .filter { (ord, p) ->
                p != jsonBodyParameter &&
                p != streamInParam &&
        ...
        val resultDeserializer = if (!isStream && method.returnType != Unit::class.createType()) {
        ...
        val streamInSerializer = if (isStream && streamInParam != null) {
            val t = streamInParam.type.arguments.first().type
                ?: error("$fullMethodName streaming parameter must be generic (Flow<T>)")
            serializer(t)
        } else null
        val streamOutSerializer = if (isStream && returnsStream) {
            val t = method.returnType.arguments.first().type
        ...
        val methodInfo = MethodInfo(
            ...
            httpMethod,
            pathBuilder,
            if (jsonBodyParameter != null) BodyParameter(bodyParamIndex, bodySerializer!!) else null,
            resultDeserializer,
            isStream = isStream,
    ...
    return Proxy.newProxyInstance(clazz.classLoader, arrayOf(clazz)) { _, method, args ->
        ...
        val jsonBody: String? = if (bodyParameter != null) {
            ...
            if (info.isBidi) {
                ...
                val flowResult: Flow<Any?> = callbackFlow {
                    val job = scope.launch {
                        try {
                            http.webSocket(urlString = wsUrl, request = {
                                    ...
                                    for (frame in incoming) {
                                        if (frame is Frame.Text) {
                                            ...
                                            if (txt.startsWith("{\"error\"")) {
                                                val env = Json.decodeFromString(ErrorEnvelope.serializer(), txt)
                ...
                if (info.isSuspend) {
                    @Suppress("UNCHECKED_CAST")
                    val ct = args.last() as Continuation<Any?>
            ...
            return@newProxyInstance callbackFlow {
                val job = scope.launch(Dispatchers.IO) {
                    ...
                    try {
                        ...
                        if (code !in 200..299) {
                            val text = (conn.errorStream ?: conn.inputStream)?.readAllBytesCompat()?.toString(Charsets.UTF_8) ?: ""
                            val ct = conn.contentType ?: ""
                            val env = tryParseError(text, ct)
                            if (env != null) throw RemoteException(env.error)
                            else throw HttpResponseException(code, if (text.isBlank()) null else text, ct)
                        }
                                ...
                                if (line!!.startsWith("data:")) {
                                    ...
                                    if (payload.isNotEmpty()) {
                                        if (payload.startsWith("{\"error\"")) {
                                            val env = Json.decodeFromString(ErrorEnvelope.serializer(), payload)
        ...
        suspend fun execute(): Any? {
            val response: HttpResponse = http.request(url) {
...
class RemoteException internal constructor(error: WireError) :
    ...
    init {
        val st = error.stack?.map {
            StackTraceElement("<remote>", it.function ?: "<fn>", it.file ?: "<none>", it.line ?: -1)
        }?.toTypedArray()
...
private fun tryParseError(text: String, ct: String?): ErrorEnvelope? = try {
    if (text.startsWith("{\"error\"")) Json.decodeFromString(ErrorEnvelope.serializer(), text) else null
