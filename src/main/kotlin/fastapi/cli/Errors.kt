...
/**
 * Typed CLI errors; can be aggregated. Keep textual payloads lean and render
 * final messages in one place to keep UX consistent.
 */
...
/**
 * Exit code policy is centralized here. This preserves legacy semantics (1 vs 2)
 * while making it trivial to change later.
 */
object ExitCodes {
    fun code(e: CliError): Int = when (e) {
        is CliError.Usage -> 2
...
fun renderCliError(err: CliError): String = when (err) {
...
private fun renderParseErrors(errors: List<ParseError>): String {
    ...
    if (errors.size == 1) {
        return when (val e = errors.first()) {
            ...
            is ParseError.OptionNeedsValue ->
                "Option ${e.name} requires a value"
    ...
    val footer = if (errors.any { it is ParseError.MissingPositionals || it is ParseError.MissingRequiredOptions })
