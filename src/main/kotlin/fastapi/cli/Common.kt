...
internal enum class ParamKind { FLAG, OPTION, POSITIONAL, STDIN_JSON, STDIN_TEXT, STDIN_BYTES }

internal enum class RepeatKind { NONE, LIST, SET, ARRAY }
...
internal fun describeInterface(kClass: KClass<*>): List<MethodSpec> {
    ...
    for (fn in kClass.members.filterIsInstance<kotlin.reflect.KFunction<*>>()) {
        ...
        val examples = fn.annotations.filterIsInstance<Cli.Example>().map { it.text }
        ...
        for (p in fn.valueParameters) {
            ...
            val repeatKind = when {
                erasure == List::class || erasure == MutableList::class || erasure == Collection::class -> RepeatKind.LIST
            ...
            val elemK: KClass<*>? = when (repeatKind) {
                RepeatKind.LIST, RepeatKind.SET, RepeatKind.ARRAY ->
                    p.type.arguments.firstOrNull()?.type?.jvmErasure
                else -> null
            }
            ...
            val isRequiredOption = (o != null) && !p.isOptional && !p.type.isMarkedNullable && repeatKind == RepeatKind.NONE
            val isRequiredPos = (pos != null) && !p.isOptional && !p.type.isMarkedNullable && repeatKind == RepeatKind.NONE

            when {
                stdinJson != null || stdinText != null || stdinBytes != null -> {
                    require(!stdinSeen) { "Only one @Stdin* parameter allowed for ${kClass.simpleName}::${fn.name}" }
                    stdinSeen = true
                    val kind = when {
...
@Suppress("UNCHECKED_CAST")
internal fun parseScalar(k: KClass<*>, raw: String): Any = when (k) {
    String::class -> raw
    Int::class -> raw.toInt()
...
private fun parseDurationBestEffort(s: String): Duration {
    // ISO-8601 first
    runCatching { return Duration.parse(s) }
    val m = Regex("""^(\d+)(ms|s|m|h)$""", RegexOption.IGNORE_CASE).matchEntire(s.trim())
