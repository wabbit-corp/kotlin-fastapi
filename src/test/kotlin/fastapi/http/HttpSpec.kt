...
interface TestInterface {
    @Http.Get("/api/test") suspend fun test(): String

    @Http.Get("/api/test/{id}") suspend fun test2(id: Int): String

    @Http.Post("/api/test") suspend fun test3(@Http.JsonBody body: String): String

    @Http.Post("/api/test/{id}") suspend fun test4(id: Int, @Http.JsonBody body: String): String
}

interface StreamApi {
    @Http.Get("/api/stream") @Http.Stream suspend fun stream(): Flow<String>

    @Http.Post("/api/echo") @Http.Stream suspend fun echo(input: Flow<String>): Flow<String>

    @Http.Get("/api/secure")
    suspend fun secure(@Http.Bearer token: String, @Http.Header("X-Trace") trace: String): String

    @Http.Get("/api/fail") suspend fun fail(): String
}
...
class HttpSpec {
    ...
    fun `test Http`() {
        runBlocking {
            val engine =
                server<TestInterface, StreamApi>(
                    ...
                    object : StreamApi {
                        ...
                        override suspend fun secure(token: String, trace: String): String =
                            "ok $token $trace"
